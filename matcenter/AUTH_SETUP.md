...
Старый режим с паролем в URL больше не поддерживается. Обновите оба deployment
до v3, убедитесь, что каждый endpoint отвечает `authVersion: 3`, и только после
этого публикуйте новую версию сайта.

## Администраторы

Доступ к `admin.html` не связан с паролем Матцентра. Аккаунт владельца
`dmb23930@gmail.com` имеет постоянный доступ по неизменяемому Firebase UID, а
не по редактируемому адресу почты. После публикации правил из
